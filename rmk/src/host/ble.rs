...
use crate::channel::{BleHostTransport, HOST_BLE_REPLY};
...
pub(crate) async fn run_ble_host<P: PacketPool>(
    hid_input: Characteristic<[u8; 32]>,
    vendor_input: Characteristic<[u8; 32]>,
    conn: &GattConnection<'_, '_, P>,
) -> ! {
    HOST_BLE_REPLY.clear();
    loop {
        let (transport, buf) = HOST_BLE_REPLY.receive().await;
        debug!("Sending via report: {:?}", buf);
        let input = match transport {
            BleHostTransport::Hid => hid_input,
            BleHostTransport::VendorGatt => vendor_input,
        };
