    ...
    async fn init(&mut self) -> Result<(), Error<I::Error>> {
        ...
        let (system_config_0, system_config_1, i2c_timeout_ms, active_interval_ms) = match &mut self.window_detection {
            WindowDetection::Rdy(_) => (
                0b01100100, // §8.10.9:  !MANUAL_CONTROL | SETUP_COMPLETE | WDT | REATI
                0b00001111, // §8.10.10: REATI_EVENT | TP_EVENT | GESTURE_EVENT | EVENT_MODE
                30, 9,
            ),
            WindowDetection::Poll { interval_ms, .. } => (
                0b11100100, // §8.10.9: MANUAL_CONTROL | SETUP_COMPLETE | WDT | REATI
                0,          // §8.10.10: !EVENT_MODE
                100,
                *interval_ms,
            ),
        };
